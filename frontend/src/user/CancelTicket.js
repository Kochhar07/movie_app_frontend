...
export const CancelTicket = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/ticket/${user._id}/view_ticket`, {
        headers: {
      ...
      .then((response) => {
        setPost({ ...post, view_tickets: response.data.seats });
      });
  }, []);
  ...
  const onClick = (e) => {
    ...
    axios
      .delete(
        `http://localhost:5000/api/ticket/${user._id}/cancel_ticket/${obj}`,
        {
          headers: {
